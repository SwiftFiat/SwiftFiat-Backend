...
type Config struct {
	...
	MinCardAmount             float64  `mapstructure:"MINIMUM_CARD_AMOUNT"`
	MaxCardAmount             float64  `mapstructure:"MAXIMUM_CARD_AMOUNT"`
	MaxCardsPerUser           int      `mapstructure:"MAXIMUM_CARDS_PER_USER"`
	DefaultCardCurrency       string   `mapstructure:"DEFAULT_CARD_CURRENCY"`
	FundingFeePct             float64  `mapstructure:"FUNDING_FEE_PCT"`
	WithdrawalFeePct          float64  `mapstructure:"WITHDRAWAL_FEE_PCT"`
...
func LoadCustomConfig(path string, val interface{}) error {
	...
	_ = v.BindEnv("PLUNK_BASE_URL")
	_ = v.BindEnv("PLUNK_SECRET_KEY")
	_ = v.BindEnv("VT_PASS_SANDBOX_URL")
